...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('rankingfem')
    .setDescription('Ver ranking feminino'),
  ...
  async execute(interaction) {
    ...
    if (res.rows.length === 0) {
      return interaction.reply('❌ Ranking feminino vazio.');
    }
    ...
    const gerarEmbed = (pagina) => {
      ...
      return new EmbedBuilder()
        .setColor('#FF1493') // 💖 rosa mais forte
        .setTitle('💖 Ranking Feminino')
    ...
    const gerarBotoes = () => {
      return new ActionRowBuilder().addComponents(
        new ButtonBuilder()
          .setCustomId('rankfem_prev')
          .setLabel('⬅️')
          .setStyle(ButtonStyle.Secondary)
          .setDisabled(pagina === 0),

        new ButtonBuilder()
          .setCustomId('rankfem_next')
          .setLabel('➡️')
          .setStyle(ButtonStyle.Secondary)
    ...
    collector.on('collect', async i => {
      ...
      if (i.customId === 'rankfem_next') pagina++;
      if (i.customId === 'rankfem_prev') pagina--;
